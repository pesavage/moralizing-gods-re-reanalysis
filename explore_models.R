...
#expect_true(abs(year_appear_50_mean - (-950)) < 50) #commented these out as they are no longer true after modifying assumptions
#expect_true(abs(year_appear_50_se - (200)) < 50) #commented these out as they are no longer true after modifying assumptions
...
polygon(
  c(-958 + c(-1.96, 1.96) * 210,
    -958 + c(1.96, -1.96) * 210),
  c(0, 0, 1, 1),
  border = NA,
  col = col.alpha("lightblue", 0.3)
)

abline(v = -958, col = "lightblue")

text(-958, 0.7, "Beheim et al. predicted\nMG emergence", srt = 90)
text(year_appear_50_mean, 0.78, "revised predicted\nMG emergence", srt = 90)

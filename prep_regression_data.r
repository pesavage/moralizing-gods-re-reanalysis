...
data <- transform(data, MG_missing = ifelse(Writing == 1, 0, MG_missing)) #This is key modification setting any polity with moralizing gods unknown but writing present as moralizing gods known absent
data <- transform(data, MHG_missing = ifelse(Writing == 1, 0, MHG_missing)) #This is key modification setting any polity with moralizing gods unknown but writing present as moralizing gods known absent
data <- transform(data, BSP_missing = ifelse(Writing == 1, 0, BSP_missing)) #This is key modification setting any polity with moralizing gods unknown but writing present as moralizing gods known absent


data[is.na(data)] <- 0 
# This treats NA values as 0
